...
def get_posix_attr_from_ldap_backend(logger, ldb_object, base_dn, user, attr):
    ...
    PATCHED: utilise python-ldap au lieu de ldb_ildap (bug Samba 4.20+)
    """
    import ldap as pyldap
    try:
        results = ldb_object.search_s(
            base_dn,
            pyldap.SCOPE_SUBTREE,
            "(&(objectClass=posixAccount)(uid=%s))" % user,
            [attr]
        )
        ...
        if len(results) == 0:
            ...
            if attr not in entry:
                raise KeyError
            return entry[attr][0].decode('utf-8') if isinstance(entry[attr][0], bytes) else entry[attr][0]
        else:
...
"""
    ...
    pgids = {}
    if ldap:
        # PATCH: contournement bug ldb_ildap.c:923 (Samba 4.20+)
        # Utilisation de python-ldap au lieu du driver ildap cassé
        import ldap as pyldap
        urls = samba3.lp.get("passdb backend").split(":", 1)[1].strip('"')
        url = urls.split()[0]
        try:
            ldb_object = pyldap.initialize(url)
            ldb_object.simple_bind_s(ldapuser, ldappass)
            ldb_object._base_dn = base_dn
            ldb_object._is_pyldap = True
        except pyldap.LDAPError as e:
            raise ProvisioningError("Could not open ldap connection to %s, the error message is: %s" % (url, e))
